...
class RigFeetModel(rigBase.BaseModel):
    def __init__(self):
        super(RigFeetModel, self).__init__()
        

class RigFeet(rigBase.RigBase):
    def __init__(self, *args, **kwargs):
        super(RigFeet, self).__init__(*args, **kwargs)
    ...
    def StandardFeetIKRig(self, StandarFeetPointsDic, FeetControl=None):
        ...
        RMRigTools.RMConnectWithLimits("%s.ToeLift" % FeetControl, "%s.rotateZ" % BallLift,
                                       [[-10, -70], [0, 0], [10, 70]])
        RMRigTools.RMConnectWithLimits("%s.BallPivot" % FeetControl, "%s.rotateZ" % BallGrp,
                                       [[-10, 70], [0, 0], [10, -70]])
        RMRigTools.RMConnectWithLimits("%s.HeelPivot" % FeetControl, "%s.rotateZ" % TapGrp,
                                       [[-10, -70], [0, 0], [10, 70]])
        RMRigTools.RMConnectWithLimits("%s.ToePivot" % FeetControl, "%s.rotateZ" % TipGrp,
                                       [[-10, 70], [0, 0], [10, -70]])
        if (Side == "L"):

...
class RigSpaceSwitch(rigBase.RigBase):
    ...
    def create_point_base(self, *points, **kwargs):
        ...
        control_object = kwargs.pop('control_object', None)
        ...
        if len(space_objects) == 2:
            if switch_type == "enum":
                ...
                for eachObject in space_objects:
                    SpaceObjectShortName.append(self.name_convention.get_a_short_name(eachObject))
                self.add_enum_parameters(SpaceObjectShortName, control_object, Name=attribute_name)
            else:
                if attribute_name == "":
                    index = 0
                    SwitchName = 'SW'
                ...
                self.AddNumericParameter(control_object, Name=attribute_name)

...
class RMBookPage(object):
    ...
    def CreateBookRig(self,AffectedObject):
        ...
        self.flare2, self.flareHandle2 = cmds.nonLinear(type = 'flare', lowBound = 0, highBound = self.widthValue, name = "FlareLeafsThick")
        ...
        cmds.select ( affected )
        self.flare, self.flareHandle = cmds.nonLinear(type = 'flare', lowBound = 0, highBound = self.heightValue, name = "FlareBorderRefinement")#endFlareX
        ...
        cmds.select ( affected )
        self.bendSpread, self.bendHandleSpread= cmds.nonLinear(type = 'bend', lowBound = -self.widthValue , highBound =self.widthValue , curvature = 0, name = "bendSpread")#curvature
        ...
        self.bendMidle, self.bendHandleMiddle = cmds.nonLinear(type = 'bend', lowBound = 0 , highBound = 1 , curvature = 0,name = "bendCenter")#curvature Hight Bound 
        ...
        self.bendOpen, self.bendHandleOpen = cmds.nonLinear(type = 'bend', lowBound = 0 , highBound = 1 , curvature = 0,name = "bendOpen")#curvature Hight Bound 
    ...
    def AddAttributes(self,ControlBook):
        cmds.addAttr(ControlBook,longName="BookSpread",keyable=1,hasMinValue=1,hasMaxValue=1,minValue=0,maxValue=10)
        cmds.addAttr(ControlBook,longName="BookLeaf",keyable=1,hasMinValue=1,hasMaxValue=1,minValue=-10,maxValue=10)
        cmds.addAttr(ControlBook,longName="BorderRefinementIn",keyable=1,hasMinValue=1,hasMaxValue=1,minValue=-10,maxValue=10)
        cmds.addAttr(ControlBook,longName="BorderRefinementOut",keyable=1,hasMinValue=1,hasMaxValue=1,minValue=-10,maxValue=10)
        ...
        cmds.addAttr(ControlBook,longName="OpenBendCounterOffset",keyable=1,hasMinValue=1,hasMaxValue=1,minValue=-10,maxValue=10)
    ...
    def LinkAttributes(self,ControlBook ):
        ...
        RMRigTools.RMConnectWithLimits( ControlBook + ".BookLeaf",             self.flare     + ".endFlareX",[[-10,-1],[0,1],[10,2]] )
        RMRigTools.RMConnectWithLimits( ControlBook + ".BorderRefinementOut",  self.flare2      + ".startFlareX",[[-10,-1],[0,1],[10,2]] )
        RMRigTools.RMConnectWithLimits( ControlBook + ".BorderRefinementIn",   self.flare2      + ".endFlareX",[[-10,-1],[0,1],[10,2]] )
        RMRigTools.RMConnectWithLimits( ControlBook + ".OpenBend",             self.bendOpen   + ".curvature",[[-10,-180],[0,0],[10,180]] )
        #RMRigTools.RMConnectWithLimits( ControlBook + ".OpenBend",            self.bendOpenOposit   + ".curvature",[[-10,-180],[0,0]] )
        RMRigTools.RMConnectWithLimits( ControlBook + ".OpenLength",           self.bendHandleOpen   + ".scale",[[-10,0],[0,self.heightValue/2],[10,self.heightValue]] )
        RMRigTools.RMConnectWithLimits( ControlBook + ".OpenBendCounter",      self.bendMidle  + ".curvature", [[-10,-180],[0,0],[10,180]] )
        RMRigTools.RMConnectWithLimits( ControlBook + ".OpenBendCounterOffset",self.bendHandleMiddle  + ".translateY", [[-10,-self.heightValue/2],[0,0],[10,self.heightValue/2]] )
        RMRigTools.RMConnectWithLimits( ControlBook + ".OpenBendCounterLenght",self.bendHandleMiddle  + ".scale",[[-10,0],[0,self.heightValue/2],[10,self.heightValue]] )

...
class RMVisibilitySwitch(object):
    ...
    def RemoveAffectedObject (self, ControlObject, removeObjectList , VisibilitySwitch = 'visibility', visibilityType = "visibility" ):
        '''visibilityType  valid values are overrideVisibility, lodVisibility or visibility'''
        ...
        for eachObject in removeObjectList:
            if eachObject in AffectedObjectList:
                cmds.disconnectAttr( ("%s.%s"%(ControlObject,VisibilitySwitch)) , ("%s.%s" % (eachObject,visibilityType)))

    def AddAffectedObject ( self, ControlObject, addObjectList , VisibilitySwitch = 'visibility', visibilityType = "visibility"):
        '''visibilityType  valid values are overrideVisibility, lodVisibility or visibility'''
        for eachObject in addObjectList:
            inputConnections = cmds.listConnections("%s.%s" % (eachObject,visibilityType), plugs=True, destination = True)
            if inputConnections:
                if len(inputConnections) > 0:
                    cmds.disconnectAttr(inputConnections[0], "%s.%s" % (eachObject,visibilityType))
            cmds.connectAttr( "%s.%s"%(ControlObject,VisibilitySwitch), "%s.%s" % (eachObject,visibilityType), force = True)
